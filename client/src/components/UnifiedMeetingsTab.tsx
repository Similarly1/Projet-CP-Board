...
export const UnifiedMeetingsTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 2. CORPS PRINCIPAL : DEUX VOLETS MASTER-DETAIL AVEC RETRACTATION */}
      <div className={`grid grid-cols-1 ${isSidebarCollapsed ? '' : 'lg:grid-cols-12'} gap-6 items-start`}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className="p-1.5 rounded-lg text-xs text-slate-400 hover:text-white bg-slate-800/80 hover:bg-slate-700 border border-slate-700/60 transition-all"
                  title="Réduire la liste des séances pour maximiser l'espace d'écriture"
                >
        ...
        {/* VOLET DROIT : EN UN COUP D'ŒIL (OJ & PV) */}
        <div className={`${isSidebarCollapsed ? 'w-full' : 'lg:col-span-8'} space-y-4`}>
          {isSidebarCollapsed && (
            <div className="flex items-center justify-between pb-1">
              <button
                onClick={() => setIsSidebarCollapsed(false)}
                className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-slate-700 text-xs font-medium text-slate-300 hover:text-white transition-all shadow-sm group"
                title="Déplier la liste des séances"
              >
                <PanelLeftOpen className="w-4 h-4 text-indigo-400 group-hover:scale-110 transition-transform" />
                <span>Afficher la liste des séances</span>
              </button>
            </div>
          )}
          {isLoadingDetails ? (
